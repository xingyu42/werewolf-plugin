...
参见 `CLAUDE.md` 关键约定，重点：
- `apps/` 的 `reg` 正则是用户接口契约，不可破坏兼容性
- 状态转换必须通过 `StateMachine`，违反 `StateTransitions` 的转换会被静默拒绝
- `RoleFactory.createRole` 四参数签名（roleName, game, player, e）
- `WolfRole.cleanup()` 必须在每局结束调用，否则跨局污染
- 配置双层合并（default_config + config）
