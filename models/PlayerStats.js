 ...
 * @pos 模型层 - 玩家统计，持久化战绩数据
 *
 * @dependencies 无（运行时注入 Data, redis）
 */
import { CAMPS, ROLE_CAMPS } from './Constants.js'
...
class PlayerStats {
  ...
  /**
   * 更新游戏结束后所有玩家的统计数据
   * @param {import('./Game.js').Game} game - 结束的游戏实例
  ...
  updateStats (game, result) {
    ...
    for (const [playerId, player] of game.players) {
      ...
      const playerCamp = ROLE_CAMPS[player.role]
      const playerRoleName = role.constructor.name

      let didWin = false
      if (winningCamp === '狼人' || winningCamp === CAMPS.WOLF) {
        didWin = playerCamp === CAMPS.WOLF
      } else if (winningCamp === '好人') {
        didWin = playerCamp !== CAMPS.WOLF
      } else {
        didWin = playerCamp === winningCamp
      }
